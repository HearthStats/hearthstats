class Deck < ActiveRecord::Base
  ...
  has_many :match_decks, dependent: :destroy
  has_many :matches, through: :match_decks, dependent: :destroy, inverse_of: :deck
  ...
  def update_unique_deck_stats
    if matches.count.modulo(10) == 0
      Rails.cache.delete('deck_stats' + self.id.to_s + self.current_version.try(:version).to_s)
      UniqueDeck.update_stats(unique_deck_id) if unique_deck_id
    end
  end
